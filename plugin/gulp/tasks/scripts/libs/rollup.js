...
const modules = new Set( ['es', 'amd', 'cjs', 'iife', 'umd', 'system'] )

const isInMod = ( format ) => modules.has( format )
...
const inside = ( opts ) => {
    ...
    async( file, encoding, callback ) => {
        if ( file.isStream() ) {
            this.emit( 'Error', new Error( `${PLUGIN_NAME}:  Unsupported file type: Stream!` ) )
            callback()
        }
        ...
        const bundles = await Promise.all( opts.map( async( opt ) => {
            ...
            io.input = io.input ? path.relative( v_dir, io.inputv ) : _inp
